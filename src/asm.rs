...
#[derive(Clone, Copy, Debug, Hash, PartialEq, Eq, PartialOrd, Ord, clap::ValueEnum)]
pub enum Reg {
    Rax,
    Rbx,
    Rdx,
...
impl fmt::Display for LeaArgs {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}, {}", self.reg, self.label)
    }
}

impl fmt::Display for RelLabel {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let RelLabel { label, offset } = self;
        match offset {
            Some((reg, offset)) => write!(f, "[rel {} + {} * {}]", label, reg, offset),
            None => write!(f, "[rel {}]", label),
        }
    }
}
